...
//列表查询信息api
const action =ref(requestMethodFood.page)
//使用抽屉共用的方法-查看功能共用方法
...
//列表操作信息
const operation=ref<OperationProps<FoodVo>[]>([
  {
    title:'详情',
    action: (vo:FoodVo) =>useDrawerMethod.open(vo)
  },
  {
    title:'编辑',
    action:(vo:FoodVo)=>useMangeMethod.handleUpdate(vo)
  },
  {
    title: '删除',
    danger: true,
    action: (vo: FoodVo) => {
      requestMethodFood.delete(vo.id).then(res => {

...
//列表查询信息api
const action =ref(requestMethodExercise.page)
//使用抽屉共用的方法-查看功能共用方法
...
//列表操作信息
const operation=ref<OperationProps<ExerciseVo>[]>([
  {
    title:'详情',
    action: (vo:ExerciseVo) =>useDrawerMethod.open(vo)
  },
  {
    title:'编辑',
    action:(vo:ExerciseVo)=>useMangeMethod.handleUpdate(vo)
  },
  {
    title: '删除',
    danger: true,
    action: (vo: ExerciseVo) => {
      requestMethodExercise.delete(vo.id).then(res => {

<template>
  ...
  >
    <a-form-item label="活动标题" name="activityTitle">
      <a-input v-model:value="formState.activityTitle" placeholder="请输入活动标题" allow-clear />
    </a-form-item>
    <a-form-item label="活动内容" name="activityIntroduction">
      <a-input v-model:value="formState.activityIntroduction" placeholder="请输入活动内容" allow-clear />
    </a-form-item>
    <a-form-item label="活动地点" name="activityPlace">
      <a-textarea v-model:value="formState.activityPlace" placeholder="请输入活动地点" allow-clear />
    </a-form-item>
    ...
    <a-form-item label="活动类型" name="activityTypeName">
      <a-input v-model:value="formState.activityTypeName" placeholder="请输入活动类型" allow-clear/>
    </a-form-item>
    <a-form-item label="最大人数" name="activityMaxAmount">
      <a-input v-model:value="formState.activityMaxAmount" placeholder="请输入最大人数" />
    </a-form-item>
    <a-form-item label="活动负责人" name="activityPrincipal">
      <a-input v-model:value="formState.activityPrincipal" placeholder="请输入活动负责人" />
    </a-form-item>
    <a-form-item label="活动负责人电话" name="activityMobile">
      <a-input v-model:value="formState.activityMobile" placeholder="请输入活动负责人电话" />
    </a-form-item>
...
import requestMethodExercise from '../../../api/menu/exercise'
...
const formRules = ref({
  activityPlace: [{ required: true, message: '当前项不能为空' }],
  activityTypeName: [{ required: true, message: '当前项不能为空' }],
})
//上传文件列表
...
watch(
    ...
    () => {
      ...
      formState.activityCover = doneFile.map(value => value?.uploadPath).join(',')
      console.log(formState.activityCover)
    },
    { immediate: true }
)
...
async function onFinish(params: ExerciseVo) {
  ...
  if (props.id) {
    //提交编辑表单
    const { message } = await requestMethodExercise.update( { ...data,id:props.id})
    messageMethod.success(message)
    useUpdateMethod.handleSave()
  } else {
    //提交表单
    const { message } = await requestMethodExercise.add(data)
    messageMethod.success(message)
    useUpdateMethod.handleSave()
  }
}
...
async function getDetailById() {
  if (props.id) {
    const { data } = await  requestMethodExercise.getByExerciseId(props.id)
    Object.assign(formState, data)
    //数据回显
    fileList.value = formState?.activityCover?.split(',').map((file, fileIndex) => {
      return {

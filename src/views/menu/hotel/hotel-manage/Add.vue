<template>
  ...
  >
    <a-form-item label="酒店名称" name="hotelTitle">
      <a-input v-model:value="formState.hotelTitle" placeholder="请输入酒店名称" allow-clear />
    </a-form-item>
    <a-form-item label="酒店等级" name="hotelGrade">
      <a-input v-model:value="formState.hotelGrade" placeholder="请输入酒店等级" allow-clear />
    </a-form-item>
    ...
    <a-form-item label="酒店价格" name="hotelPrice">
      <a-input v-model:value="formState.hotelPrice" placeholder="请输入酒店价格" allow-clear/>
    </a-form-item>
    <a-form-item label="酒店地址" name="hotelAddress">
      <a-textarea v-model:value="formState.hotelAddress" placeholder="请输入酒店地址" allow-clear/>
    </a-form-item>
...
const formRules = ref({
  hotelTitle: [{ required: true, message: '当前项不能为空' }],
  hotelGrade: [{ required: true, message: '当前项不能为空' }],
  hotelPic:[{ required: true, message: '当前项不能为空' }],
})
//上传文件列表
...
watch(
    ...
    () => {
      ...
      formState.hotelPic = doneFile.map(value => value?.uploadPath).join(',')
      console.log(formState.hotelPic)
    },
    { immediate: true }
)
...
async function getDetailById() {
  if (props.id) {
    ...
    //数据回显
    fileList.value = formState?.hotelPic?.split(',').map((file, fileIndex) => {
      return {

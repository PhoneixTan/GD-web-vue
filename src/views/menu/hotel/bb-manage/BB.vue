...
//列表查询信息api
const action =ref(requestMethodBb.page)
//使用抽屉共用的方法-查看功能共用方法
...
//列表操作信息
const operation=ref<OperationProps<BbVo>[]>([
  {
    title:'详情',
    action: (vo:BbVo) =>useDrawerMethod.open(vo)
  },
  {
    title:'编辑',
    action:(vo:BbVo)=>useMangeMethod.handleUpdate(vo)
  },
  {
    title: '删除',
    danger: true,
    action: (vo: BbVo) => {
      requestMethodBb.delete(vo.id).then(res => {

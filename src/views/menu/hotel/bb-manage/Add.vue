<template>
  ...
  >
    <a-form-item label="民宿名称" name="bbTitle">
      <a-input v-model:value="formState.bbTitle" placeholder="请输入民宿名称" allow-clear />
    </a-form-item>
    <a-form-item label="民宿等级" name="bbGrade">
      <a-input v-model:value="formState.bbGrade" placeholder="请输入民宿等级" allow-clear />
    </a-form-item>
    ...
    <a-form-item label="民宿价格" name="bbPrice">
      <a-input v-model:value="formState.bbPrice" placeholder="请输入民宿价格" allow-clear/>
    </a-form-item>
    <a-form-item label="民宿地址" name="bbAddress">
      <a-textarea v-model:value="formState.bbAddress" placeholder="请输入民宿地址" allow-clear/>
    </a-form-item>
    <a-form-item label="民宿介绍" name="bbIntroduce">
      <a-textarea v-model:value="formState.bbIntroduce" placeholder="请输入民宿介绍" allow-clear/>
    </a-form-item>
...
const formRules = ref({
  bbTitle: [{ required: true, message: '当前项不能为空' }],
  bbGrade: [{ required: true, message: '当前项不能为空' }],
  bbPic:[{ required: true, message: '当前项不能为空' }],
})
//上传文件列表
...
watch(
    ...
    () => {
      ...
      formState.bbPic = doneFile.map(value => value?.uploadPath).join(',')
      console.log(formState.bbPic)
    },
    { immediate: true }
)
...
async function getDetailById() {
  if (props.id) {
    ...
    //数据回显
    fileList.value = formState?.bbPic?.split(',').map((file, fileIndex) => {
      return {

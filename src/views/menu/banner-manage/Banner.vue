...
//列表查询信息api
const action =ref(requestMethodBanner.page)
//使用抽屉共用的方法-查看功能共用方法
...
//列表操作信息
const operation=ref<OperationProps<BannerVo>[]>([
  {
    title:'详情',
    action: (vo:BannerVo) =>useDrawerMethod.open(vo)
  },
  {
    title:'编辑',
    action:(vo:BannerVo)=>useMangeMethod.handleUpdate(vo)
  },
  {
    title: '删除',
    danger: true,
    action: (vo: BannerVo) => {
      requestMethodBanner.delete(vo.id).then(res => {

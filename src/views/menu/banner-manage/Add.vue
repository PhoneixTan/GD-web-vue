<template>
  ...
  >
    <a-form-item label="轮播图标题" name="bannerTitle">
      <a-input v-model:value="formState.bannerTitle" placeholder="请输入轮播图标题" allow-clear />
    </a-form-item>
    ...
    <a-form-item label="轮播图点赞数" name="bannerLattice">
      <a-input v-model:value="formState.bannerLattice" placeholder="请输入轮播图点赞数" allow-clear/>
    </a-form-item>
...
const formRules = ref({
  bannerPic: [{ required: true, message: '当前项不能为空' }],
})
//上传文件列表
...
watch(
    ...
    () => {
      ...
      formState.bannerPic = doneFile.map(value => value?.uploadPath).join(',')
      console.log(formState.bannerPic)
    },
    { immediate: true }
)
...
async function getDetailById() {
  if (props.id) {
    ...
    //数据回显
    fileList.value = formState?.bannerPic?.split(',').map((file, fileIndex) => {
      return {

...
const action=ref(requestMethodProposal.page)
//使用抽屉-查看功能共用方法
...
//操作信息
const operation=ref<OperationProps<SuggestVo>[]>([
  {
    title:'详情',
    action: (vo:SuggestVo) =>useDrawerMethod.open(vo)
  },
  {
    title:'回复',
    action: (vo:SuggestVo) =>useMangeMethod.handleUpdate(vo)
  },
  {
    title:'删除',
    danger:true,
    action:(vo:SuggestVo) => {
      requestMethodProposal.delete(vo.id).then(res =>{

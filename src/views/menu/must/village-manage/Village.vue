...
//列表查询信息api
const action =ref(requestMethodVillage.page)
//使用抽屉共用的方法-查看功能共用方法
...
//列表操作信息
const operation=ref<OperationProps<VillageVo>[]>([
  {
    title:'详情',
    action: (vo:VillageVo) =>useDrawerMethod.open(vo)
  },
  {
    title:'编辑',
    action:(vo:VillageVo)=>useMangeMethod.handleUpdate(vo)
  },
  {
    title: '删除',
    danger: true,
    action: (vo: VillageVo) => {
      requestMethodVillage.delete(vo.id).then(res => {

<template>
  ...
  >
    <a-form-item label="景区村名称" name="villageName">
      <a-input v-model:value="formState.villageName" placeholder="请输入景区村名称" allow-clear />
    </a-form-item>
    <a-form-item label="景区村等级" name="villageGrade">
      <a-input v-model:value="formState.villageGrade" placeholder="请输入景区村等级" allow-clear />
    </a-form-item>
    <a-form-item label="景区村描述" name="villageEvaluate">
      <a-input v-model:value="formState.villageEvaluate" placeholder="请输入景区村描述" allow-clear />
    </a-form-item>
    <a-form-item label="景区村地址" name="villageAddress">
      <a-textarea v-model:value="formState.villageAddress" placeholder="请输入景区村地址" allow-clear />
    </a-form-item>
    ...
    <a-form-item label="景区村介绍" name="villageIntroduce">
      <a-textarea v-model:value="formState.villageIntroduce" placeholder="请输入景区村介绍" allow-clear/>
    </a-form-item>
    <a-form-item label="当前高峰" name="villagePeak">
      <a-input v-model:value="formState.villagePeak" placeholder="请输入当高峰" allow-clear/>
    </a-form-item>
    <a-form-item label="累计人数" name="villageTotal">
      <a-input v-model:value="formState.villageTotal" placeholder="请输入累计人数" />
    </a-form-item>
    <a-form-item label="景区村客流量" name="villageVisitor">
      <a-input v-model:value="formState.villageVisitor" placeholder="请输入景区村客流量" />
    </a-form-item>
...
const formRules = ref({
  villageName: [{ required: true, message: '当前项不能为空' }],
  villageGrade: [{ required: true, message: '当前项不能为空' }],
})
//上传文件列表
...
watch(
    ...
    () => {
      ...
      formState.villagePic = doneFile.map(value => value?.uploadPath).join(',')
      console.log(formState.villagePic)
    },
    { immediate: true }
)
 ...
 * 表单提交并数据验证成功
 * */
async function onFinish(params:VillageVo) {
  const data = {
...
async function getDetailById() {
  if (props.id) {
    ...
    //数据回显
    fileList.value = formState?.villagePic?.split(',').map((file, fileIndex) => {
      return {

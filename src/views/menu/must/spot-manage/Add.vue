<template>
  ...
  >
    <a-form-item label="景区名称" name="spotname">
      <a-input v-model:value="formState.spotname" placeholder="请输入景区名称" allow-clear />
    </a-form-item>
    <a-form-item label="景区等级" name="grade">
      <a-input v-model:value="formState.grade" placeholder="请输入景区等级" allow-clear />
    </a-form-item>
    <a-form-item label="景区描述" name="miaosu">
      <a-input v-model:value="formState.miaosu" placeholder="请输入景区描述" allow-clear />
    </a-form-item>
    <a-form-item label="景区地址" name="spotAddress">
      <a-textarea v-model:value="formState.spotAddress" placeholder="请输入活动地点" allow-clear />
    </a-form-item>
    ...
    <a-form-item label="景区介绍" name="spotIntroduce">
      <a-textarea v-model:value="formState.spotIntroduce" placeholder="请输入景区介绍" allow-clear/>
    </a-form-item>
    <a-form-item label="当前客流数量" name="numberOne">
      <a-input v-model:value="formState.numberOne" placeholder="请输入当前客流数量" allow-clear/>
    </a-form-item>
    <a-form-item label="入园高峰数量" name="numberThree">
      <a-input v-model:value="formState.numberThree" placeholder="请输入入园高峰数量" />
    </a-form-item>
    <a-form-item label="今日累计流量" name="numberTwo">
      <a-input v-model:value="formState.numberTwo" placeholder="请输入今日累计流量" />
    </a-form-item>
...
const formRules = ref({
  spotname: [{ required: true, message: '当前项不能为空' }],
  grade: [{ required: true, message: '当前项不能为空' }],
  miaosu:[{ required: true, message: '当前项不能为空' }],
  numberOne:[{ required: true, message: '当前项不能为空' }],
  numberThree:[{ required: true, message: '当前项不能为空' }],
  numberTwo:[{ required: true, message: '当前项不能为空' }],
...
watch(
    ...
    () => {
      ...
      formState.photo = doneFile.map(value => value?.uploadPath).join(',')
...
async function getDetailById() {
  if (props.id) {
    ...
    //数据回显
    fileList.value = formState?.photo?.split(',').map((file, fileIndex) => {
      return {

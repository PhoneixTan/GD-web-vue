...
//列表查询信息api
const action =ref(requestMethodSpot.page)
//使用抽屉共用的方法-查看功能共用方法
...
//列表操作信息
const operation=ref<OperationProps<SpotVo>[]>([
  {
    title:'详情',
    action: (vo:SpotVo) =>useDrawerMethod.open(vo)
  },
  {
    title:'编辑',
    action:(vo:SpotVo)=>useMangeMethod.handleUpdate(vo)
  },
  {
    title: '删除',
    danger: true,
    action: (vo: SpotVo) => {
      requestMethodSpot.delete(vo.id).then(res => {

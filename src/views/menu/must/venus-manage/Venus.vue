...
//列表查询信息api
const action =ref(requestMethodVenus.page)
//使用抽屉共用的方法-查看功能共用方法
...
//列表操作信息
const operation=ref<OperationProps<VenusVo>[]>([
  {
    title:'详情',
    action: (vo:VenusVo) =>useDrawerMethod.open(vo)
  },
  {
    title:'编辑',
    action:(vo:VenusVo)=>useMangeMethod.handleUpdate(vo)
  },
  {
    title: '删除',
    danger: true,
    action: (vo: VenusVo) => {
      requestMethodVenus.delete(vo.id).then(res => {
